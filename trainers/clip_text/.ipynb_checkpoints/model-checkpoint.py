...
class ResidualAttentionBlock_Visual(nn.Module):
    ...
    def forward(self, inputs):
        if len(inputs)==6:
            ...
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            return [x, compound_prompts_deeper, n_ctx,layers,counter,ind_w]  # return again as a list, so that nn.seq can work
        else:
            x = inputs
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,VPT_ctx=None, VPT_prompts=None, layers=None, weight=3):
